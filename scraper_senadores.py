...
def calcular_kpis(df_nomina: pd.DataFrame, df_actas: pd.DataFrame) -> pd.DataFrame:
    ...
    if col_senador and col_voto:
        ...
        col_nom = next((c for c in df_nomina.columns if "nombre" in c.lower()), None)
        if col_nom:
            df_result = df_nomina.merge(resumen, left_on=col_nom, right_on="nombre", how="left")
        else:
            df_result = df_nomina.copy()

        if "total_votos" in df_result.columns:
...
def reporte_provincial(df: pd.DataFrame) -> pd.DataFrame:
    col_prov = next(
        (c for c in df.columns if any(k in c.lower() for k in ["provincia", "distrito"])),
        None
    )
    if not col_prov:
        return pd.DataFrame()
    ...
    resumen = df.groupby(col_prov).agg(agg).reset_index()
    resumen.rename(columns={df.columns[0]: "senadores"}, inplace=True)
...
# ── 5. Guardar ────────────────────────────────────────────────────────────────
def guardar_resultados(df_sen: pd.DataFrame, df_prov: pd.DataFrame):
...
def main():
    ...
    guardar_resultados(df_final, df_provincial)

    print("\n" + "=" * 60)
    print("📌 RESUMEN EJECUTIVO")
    print("=" * 60)

    col_bloque = next((c for c in df_final.columns if "bloque" in c.lower()), None)
    if col_bloque:
        print(f"\n🏛️  Por Bloque:\n{df_final[col_bloque].value_counts().to_string()}")

    col_prov = next((c for c in df_final.columns if "provincia" in c.lower()), None)
    if col_prov:
        print(f"\n🗺️  Provincias: {df_final[col_prov].nunique()}")

    if not df_provincial.empty:
        print(f"\n📊 Top Provincias:\n{df_provincial.head().to_string(index=False)}")
